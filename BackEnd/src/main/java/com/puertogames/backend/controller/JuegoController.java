...
public class JuegoController {
     ...
     * GET /api/juegos/categorias
     */
    @GetMapping("/categorias")
    public ResponseEntity<List<String>> obtenerCategorias() {
        List<String> categorias = juegoService.obtenerTodasCategorias();
        return ResponseEntity.ok(categorias);
    }
    
    /**
     * Filtra juegos con múltiples criterios
     * GET /api/juegos/filtrar?categoria=Action&consola=PS5&anoMin=2020&anoMax=2024
     */
    @GetMapping("/filtrar")
    public ResponseEntity<List<JuegoResponseDTO>> filtrarJuegos(
        ...
        List<JuegoResponseDTO> response = juegoService.filtrarJuegos(categoria, consola, anoMin, anoMax);
        return ResponseEntity.ok(response);
    }
