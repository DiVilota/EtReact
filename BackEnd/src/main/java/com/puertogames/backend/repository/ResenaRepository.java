...
public interface ResenaRepository extends JpaRepository<Resena, Long> {
    ...
    // Promedio por juego
    @Query("SELECT AVG(r.rating) FROM Resena r WHERE r.juego.id = :juegoId AND r.rating IS NOT NULL")
    Double findAverageRatingByJuegoId(@Param("juegoId") Long juegoId);

    // Promedio global
    @Query("SELECT AVG(r.rating) FROM Resena r WHERE r.rating IS NOT NULL")
